...
|[asp-route-{value}](xref:mvc/views/tag-helpers/builtin-th/anchor-tag-helper#asp-route-value)|A single URL route value i.e. `asp-route-id="1234"`|
...
The following example would submit the form to the `HomeController`s `Index` action if either the input or button were clicked.
...
The following example would submit the form to the `About` Razor Page.
